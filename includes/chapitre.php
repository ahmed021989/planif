...
class Chapitre {
	...
	protected static $nom_table="chapitre";
	protected static $champs = array('id_chapitre	','nom_chapitre','id_ss');
	public $id_chapitre;
	public $nom_chapitre;
	public $code_ss;
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_chapitre ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_chapitre)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_chapitre =". $bd->escape_value($this->id_chapitre) ;
$bd->requete($sql);
