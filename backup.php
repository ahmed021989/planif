...
if(isset($_GET['task'])&& $_GET['task']=='clear'){
    ...
    if(file_exists($file)){ if(unlink($file)) $rmsg="$file_name supprimé avec succès";}
    else { $rmsg="<b>$file_name </b>Pas trouvé déjà Supprimé";}
}
...
$host= htmlentities(DB_HOST);
$password= htmlentities(DB_PASS);
$userhoste= htmlentities(DB_USER);
$database= htmlentities(DB_SCHEMA);

//if(!empty($host)&&!empty($user)&&!empty($password)&&!empty($database))
// Define  Database Credentials
define('HOST', $host ) ; 
define('USER', $userhoste ) ; 
...
*/
$fileName = 'backup-GSPI-DEP-' . date('Y').'-'.date('m').'-'.date('d') . '@'.date('H.i.s').'.sql' ; 
...
.backup_list{ background: #f9f9f9;width:100%;min-height: 410px;height: auto; padding: 10px;float:left }
...
table{width:500px;margin-bottom: 500px;border: solid 1px #ccc;border-collapse: collapse}
...
table > tbody > tr > td img:hover {width:14px;height: 3px; -moz-transition: all 0.5s ease-out; -o-transition: all 0.5s ease-out; -webkit-transition: all 0.5s ease-out; -ms-transition: all 0.5s ease-out; }
a.tooltips {  position: relative;  display: inline;}
a.tooltips span {position: absolute;width:140px;color: #000000;background: #FFFFFF;border: 4px solid #CCCCCC;height: 32px;line-height: 32px;text-align: center;visibility: hidden;border-radius: 6px;box-shadow: 0px 0px 7px #808080;}
...
.overlay-msg{margin-bottom: 10px;bottom: 0px;position: absolute;font-style: italic;color: rgb(19, 19, 19);}             
                    ...
                    <div class="row">
                        <div class="col-md-12">
                           ...
                           <form class="form-horizontal" role="form"  name="backup"   id = "getdb"  action="<?php echo $_SERVER['PHP_SELF']?>" method="post">
...
$msg='';
$msg.='    <div class="page-content-wrap">                
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="panel panel-default">
                                <div class="panel-heading">                                
									 ...
									 <h3 class="panel-title"><strong>Sauvegarde et restauration de base de données</strong></h3>
									
                                                                 
                                </div>
                                <div class="panel-body">
                                    <table id="table0" class="table datatable scrollable">
                                        <thead>
                                            <tr>
...
<a class="tooltips" href="'.BACKUP_DIR . "/". $v['name'] .'" target="_blank"><span>Télécharger Backup</span><img src="data:image/png;base64,"/></a>
&nbsp;||&nbsp;<a onclick="return confirm(\'Voulez-vous vraiment supprimer ce fichier??\')" class="tooltips" href="backup.php?task=clear&file='.$v['name'].'"><span> Supprimer </span><img src="data:image/png;base64,"/></a>
</td></tr>';   
}
return $msg.='</tbody>
</div>
...
function getDownloads($dir="./myBackups"){
 ...
 <!-- The Modal document scanner -->
                              <div class="panel-footer">
                             								    ...
                             								    <button  class ="btn btn-success  btn-block  "  onclick="vky(this)" type="submit" id="getdb" name="submit" value="Sauvegarder database" >Sauvegarder Base de Donnée </button>  
    ...
    function vky(x){
        x.value='En Cours de Traitement...';
