...
class    Projet {
	...
	protected static $nom_table="projet";
	protected static $champs = array('id_projet','nom_projet','id_infra','id_ord','date_lence','etudes','cpo','ces','vrd','date_ser','obs','eqt','date_reception','reev_dem');
	public $id_projet;
	public $nom_projet;
	public $id_infra;
	public $id_ord;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE nom_projet = '".$this->nom_projet."' ";
	...
	public static function trouve_tous() {
  	...
  	public static function trouve_filtre_ord($ord,$infra) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where id_ord=".$ord." and id_infra=".$infra."");
		//return !empty($result_array) ? array_shift($result_array) : false;
  }
  
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_projet ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  public static function trouve_par_classee($classe=0){
    ...
    return  self::trouve_par_sql($q);
	 ...
	 public static function trouve_tous_infra($id) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where id_infra=".$id." ");
  }
   	 public static function trouve_tous_ord($id) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where id_ord=".$id." ");
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_projet)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_projet =". $bd->escape_value($this->id_projet) ;
$bd->requete($sql);
