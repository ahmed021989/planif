...
class    Sous_secteur {
	...
	protected static $nom_table="sous_secteur";
	protected static $champs = array('id_ss','nom_ssecteur','id_secteur');
	public $id_ss;
	public $nom_ssecteur;
	public $id_secteur;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE nom_ssecteur = '".$this->nom_ssecteur."' ";
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_ss ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_ss)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_ss =". $bd->escape_value($this->id_ss) ;
$bd->requete($sql);
