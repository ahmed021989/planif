...
class Wilayas {
	...
	protected static $champs = array('id_w', 'wilaya', 'region' ,'superficie');
	public $id_w;
	public $wilaya;
	public $region;
public $superficie;
...
{ 
  ...
  public static function trouve_par_Nom($nom_w=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE nom_w  ={$nom_w} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
    public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_w={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
   public static function trouve_par_region($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE region  ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_w)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_w =". $bd->escape_value($this->id_w) ;
$bd->requete($sql);
