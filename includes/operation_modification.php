...
class    Operation_modif {
	
	protected static $nom_table="operation_modif";
	protected static $champs = array('id_modif','n_dec','reev','nom_oper','numero_op','a_nom_oper','a_numero_op','etat_operation','date_modif','id_op','valide','date_demande','date_valide','user','id_ord','code_type_prog');
	public $id_modif;
	public $n_dec;
	public $reev;
	public $nom_oper;
...
public $code_type_prog;
  ...
  public function nom_compler() {
	...
	public static function trouve_tous() {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where valide=0");
  }
  	public static function trouve_tous_modif() {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where valide=0 and etat_operation!='Gelee' ");
  }
  	public static function trouve_tous_nonmodif() {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where valide=1 and etat_operation!='Gelee'");
  }
  	public static function trouve_tous_reev($id_op) {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where valide=1 and id_op='".$id_op."'" );
  }
  
  public static function trouve_tous_valide_gelee() {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where valide=0 and etat_operation='Gelee'");
  }
   public static function trouve_tous_invalide_gelee() {
	
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where valide=1 and etat_operation='Gelee'");
	  
  }
  
   public static function trouve_tous_valide() {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where valide=1 AND  etat_operation!='Gelee'");
  }
  
   public static function trouve_tous_annule() {
		return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." where valide=-1");
  }
  
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_modif ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_modif)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_modif =". $bd->escape_value($this->id_modif) ;
$bd->requete($sql);
