...
class  Infrastructure {
	...
	protected static $nom_table=" infrastructure";
	protected static $champs = array('id_infra','code_infra','nom_infra','code_structure');
	public $id_infra;
	public $code_infra;
	public $nom_infra;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE code_infra = '".$this->code_infra."' ";
	//$sql .= "OR email = '".$this->email."' ";
    $sql .= "LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	}
	public  function  existe_in_projet($id,$dat1,$dat2){
	 global $bd;
	 $sql  = "SELECT * FROM projet ";
    $sql .= "WHERE id_infra = ".$id." and date_reception>='".$dat1."' and date_reception<='".$dat2."'";
	//$sql .= "OR email = '".$this->email."' ";
    $sql .= "LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_infra ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
   public static function trouve_par_code($code) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code_infra ='{$code}' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
    public static function trouve_par_structure($code){
  return self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code_structure ='".$code."' ");
		//return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_infra)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_infra =". $bd->escape_value($this->id_infra) ;
$bd->requete($sql);
