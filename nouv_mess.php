...
$doc='doc_pdf/'.$fichier;
     ...
     if( move_uploaded_file($fichier_tmp,$doc)){
	 ...
	 if (empty($errors)){
   		$message->save();
 		  $msg_positif = '<p style= "font-size: 20px; ">   Message envoyé ...  <span class="fa fa-envelope"></span>    </p><br />';
                ...
                <div class="content-frame">                                    
                    ...
                    <div class="content-frame-top">                        
                        <div class="page-title">                    
                          <h2><span class="fa fa-envelope"></span> Messagerie GSPI-DEP </h2>
                        </div>                                                                                
                    ...
                    <div class="content-frame-left">
                        ...
                        <div class="block">
                            <div class="list-group border-bottom">
                                <a href="message.php" class="list-group-item "><span class="fa fa-inbox"></span> Boite de réception <span class="badge badge-success"></span></a>
                                 <a href="mess_env.php" class="list-group-item"><span class="fa fa-rocket"></span> Messages envoyés</a>
                                  <a href="mess_supp.php" class="list-group-item"><span class="fa fa-trash-o"></span> Corbeille <span class="badge badge-default"></span></a>                            
                            </div>                        
                        </div>
                                       ...
                                       <form role="form" class="form-horizontal"   name="messagerie" id = "id" action="<?php echo $_SERVER['PHP_SELF']?>" method="post"  enctype="multipart/form-data">
                                           <div class="form-group">
                                               ...
                                               <div class="col-md-12 ">                                             
                                                    ...
                                                    <select class="form-control select" id="id_destinataire"   name="id_destinataire[]"   multiple="multiple" title="Sélectionner un destinataire" data-live-search="true"  required />
													
                                                       <?php
if($user->type=="administrateur"){
	echo "<option  value = 'tous' >tous</option>";
	  $SQL = $bd->requete("SELECT * FROM personne,ordonnateur where personne.id_ord=ordonnateur.id_ord order by ordonnateur.id_ord");
			while ($rows = $bd->fetch_array($SQL))
				{
				echo '<option  value = "'.$rows["id"].'" >'.$rows["login"].'  / '.$rows["nom_ord"].'</option>';
				}
}
//psd
if($user->type=="Admin_psd"){
	 $SQL1 = $bd->requete("SELECT * FROM personne,ordonnateur where personne.id_ord=ordonnateur.id_ord  and personne.type='administrateur' order by ordonnateur.id_ord");
			while ($rows = $bd->fetch_array($SQL1))
				{
				echo '<option  value = "'.$rows["id"].'" >'.$rows["login"].'  / '.$rows["nom_ord"].'</option>';
				}
				
	  $SQL = $bd->requete("SELECT * FROM personne,ordonnateur where personne.id_ord=ordonnateur.id_ord  and ordonnateur.id_prog=42 order by ordonnateur.id_ord");
			while ($rows = $bd->fetch_array($SQL))
				{
				echo '<option  value = "'.$rows["id"].'" >'.$rows["login"].'  / '.$rows["nom_ord"].'</option>';
				}
...
if($user->type=="Admin_psc"){
	 $SQL1 = $bd->requete("SELECT * FROM personne,ordonnateur where personne.id_ord=ordonnateur.id_ord  and personne.type='administrateur' order by ordonnateur.id_ord");
	  ...
	  $SQL = $bd->requete("SELECT * FROM personne,ordonnateur where personne.id_ord=ordonnateur.id_ord  and ordonnateur.id_prog=41 order by ordonnateur.id_ord");
...
if($user->type!="Admin_psd" and $user->type!="Admin_psc" and $user->type!="administrateur"){
	$id_prog=0;
	$ord=Ordonnateur::trouve_par_id($user->id_ord);

	  $SQL = $bd->requete("SELECT * FROM personne,ordonnateur where personne.id_ord=ordonnateur.id_ord  and ordonnateur.id_ord=".$user->id_ord." order by ordonnateur.id_ord");
			while ($rows = $bd->fetch_array($SQL))
				{
				echo '<option  value = "'.$rows["id"].'" >'.$rows["login"].'  / '.$rows["nom_ord"].'</option>';
				}
			...
			while ($rows = $bd->fetch_array($SQL1))
				...
				if($ord->id_prog==41){
				 $SQL2 = $bd->requete("SELECT * FROM personne,ordonnateur where personne.id_ord=ordonnateur.id_ord  and personne.type='Admin_psc' order by ordonnateur.id_ord");
			while ($rows = $bd->fetch_array($SQL2))
				{
				echo '<option  value = "'.$rows["id"].'" >'.$rows["login"].'  / '.$rows["nom_ord"].'</option>';
				}
				}
				if($ord->id_prog==42){
				 $SQL2 = $bd->requete("SELECT * FROM personne,ordonnateur where personne.id_ord=ordonnateur.id_ord  and personne.type='Admin_psd' order by ordonnateur.id_ord");
			while ($rows = $bd->fetch_array($SQL2))
				{
				echo '<option  value = "'.$rows["id"].'" >'.$rows["login"].'  / '.$rows["nom_ord"].'</option>';
				}
							...
							<br>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="pull-right">
                                        <button class="btn btn-danger" type = "submit" name = "submit"><span class="fa fa-envelope"></span> Envoyer </button>
                                    </div>                                    
                                </div>
                            </div>
