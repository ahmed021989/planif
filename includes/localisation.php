...
class   Localisation {
	...
	protected static $nom_table="localisation";
	protected static $champs = array('code_local','nom_local','region','superficie');
	public $code_local;
	public $nom_local;
	public $region;
	public $superficie;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE type_etab = '".$this->type_etab."' ";
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code_local ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->code_local)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE code_local =". $bd->escape_value($this->code_local) ;
$bd->requete($sql);
