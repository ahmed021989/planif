...
class    Type_programme {
	
	protected static $nom_table="type_programme";
	protected static $champs = array('id_type_prog','code_type_prog','nom_type_prog','designation','topographie');
	public $id_type_prog;
	public $code_type_prog;
	public $nom_type_prog;
	public $designation;
    public $topographie;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE code_type_prog = '".$this->code_type_prog."' ";
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_type_prog ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
    public static function trouve_par_code($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code_type_prog ='{$id}' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_type_prog)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_type_prog =". $bd->escape_value($this->id_type_prog) ;
$bd->requete($sql);
