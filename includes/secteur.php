...
class    Secteur {
	...
	protected static $nom_table="secteur";
	protected static $champs = array('id_secteur','nom_secteur');
	public $id_secteur;
	public $nom_secteur;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE nom_secteur = '".$this->nom_secteur."' ";
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_secteur ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_secteur)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_secteur =". $bd->escape_value($this->id_secteur) ;
$bd->requete($sql);
