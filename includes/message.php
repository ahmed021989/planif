...
{ 
  ...
  return round($resultat); // Arrondi la valeur
    ...
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_mess={$id} LIMIT 1");
		...
		return self::trouve_par_sql("SELECT * FROM personne,message where   message.id_destinataire=personne.id AND personne.id=".$user->id." AND id_des_supp=0 order by id_mess DESC");
  }

    	public static function   trouve_tous_id_expediteur($user) {
		return self::trouve_par_sql("SELECT * FROM personne,message where   message.id_expediteur=personne.id AND personne.id=".$user->id." AND  id_exp_supp=0 order by id_mess DESC");
  }
  
   	public static function   trouve_tous_supp_des($user) {
		return self::trouve_par_sql("SELECT * FROM personne,message where   message.id_destinataire=personne.id AND personne.id=".$user->id."  AND id_des_supp=-1 order by id_mess DESC ");
  }
  
  	public static function   trouve_tous_supp_exp($user) {
		return self::trouve_par_sql("SELECT * FROM personne,message where   message.id_expediteur=personne.id and personne.id=".$user->id."  AND  id_exp_supp=-1 order by id_mess DESC ");
  }
