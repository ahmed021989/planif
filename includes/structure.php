...
class    Structure {
	...
	protected static $nom_table="structure";
	protected static $champs = array('id_structure','code_structure','nom_structure','id_prog');
	public $id_structure;
  ...
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE code_structure = '".$this->code_structure."' ";
	$sql .= "AND id_prog = '".$this->id_prog."' ";
    $sql .= "LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	}
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_structure ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
   public static function trouve_par_code($id) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE code_structure ='{$id}' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_structure)? $this->modifier() : $this->ajouter();
	}
...
$sql .= " WHERE id_structure =". $bd->escape_value($this->id_structure) ;
$bd->requete($sql);
...
$sql = "update ".self::$nom_table." SET ";
$sql .= " id_structure = '".$this-> id_structure."' ";
