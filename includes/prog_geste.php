...
class   Prog_geste {
	...
	protected static $nom_table="prog_geste";
	protected static $champs = array('id_prog','code_prog','nom_prog_gest');
	public $id_prog;
	public $code_prog;
	public $nom_prog_gest;
    ...
    $sql .= "WHERE code_prog = '".$this->code_prog."' ";
  ...
  public static function trouve_par_id($id=0) {
    $result_array = self::trouve_par_sql("SELECT * FROM ".self::$nom_table." WHERE id_prog ={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  }
  ...
  public static function trouve_par_sql($sql="") {
	...
	*/
	...
	public function save(){
	 ...
	 return isset($this->id_prog)? $this->modifier()  : $this->ajouter() ;
	}
...
$sql .= " WHERE id_prog =". $bd->escape_value($this->id_prog) ;
$bd->requete($sql);
